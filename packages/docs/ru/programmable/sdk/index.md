---
title: Vue SDK
description: Создавайте редакторы на базе SignalForge с помощью headless-компосаблов и примитивов для Vue.
---

# Vue SDK

`@signal-forge/vue` существует для того, чтобы SignalForge мог быть чем-то большим, чем самостоятельное дизайн-приложение.

Цель — сделать SignalForge набором инструментов, который можно встроить в другие продукты, внутренние инструменты и редакторы под конкретные задачи, а не только использовать в виде единого стандартного интерфейса.

Приложение SignalForge — одна из возможных компоновок этого набора. SDK — это то, с помощью чего вы создаёте другие.
...
- **Интеграция с фреймворком**: Vue-интеграция поверх `@signal-forge/core`
