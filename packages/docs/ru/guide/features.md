...
**Копирование и вставка с Figma** — выделите узлы в Figma, <kbd>⌘</kbd><kbd>C</kbd>, переключитесь на SignalForge, <kbd>⌘</kbd><kbd>V</kbd>. Заливки, обводки, авто-макет, текст, эффекты, радиусы скругления и векторные сети сохраняются. Работает в обоих направлениях.
...
```sh
signalforge tree design.fig          # Дерево узлов
signalforge find design.fig --type TEXT  # Поиск
signalforge export design.fig -f png     # Рендеринг
signalforge analyze colors design.fig    # Аудит цветов
signalforge analyze clusters design.fig  # Повторяющиеся паттерны
...
```sh
signalforge tree                     # Текущий документ
signalforge export -f png            # Скриншот канваса
```

Все команды поддерживают `--json`. Установка: `npm install -g @signal-forge/cli`
...
**Веб** — работает на [app.signalforge.dev](https://app.signalforge.dev), устанавливается как PWA на мобильных устройствах с адаптированным интерфейсом.
...
Когда шрифт недоступен локально, SignalForge автоматически загружает его из Google Fonts. Ручная установка не требуется при открытии файлов .fig с незнакомыми шрифтами.
