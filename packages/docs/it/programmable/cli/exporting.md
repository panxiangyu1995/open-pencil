...
```sh
signalforge export design.fig                          # PNG (predefinito)
signalforge export design.fig -f jpg -s 2 -q 90       # JPG a 2×, qualità 90
signalforge export design.fig -f webp -s 3             # WEBP a 3×
signalforge export design.fig -f svg                   # SVG vettoriale
```
...
Supporta anche `--style signalforge` per il formato JSX nativo (vedi [Renderer JSX](../jsx-renderer)).
...
```sh
signalforge export -f png    # screenshot del canvas corrente
```
